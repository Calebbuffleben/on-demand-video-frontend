...
const PricingPlanComponent: React.FC<PricingPlanProps> = ({ name, price, description, features, priceId }) => {
  return (
    <div className="relative p-12 bg-white border border-silver-200 rounded-3xl shadow-lg flex flex-col w-[450px]">
      <div className="flex-1">
        <h3 className="text-2xl font-bold text-scale-900">{name}</h3>
        <p className="mt-6 flex items-baseline text-scale-900">
          <span className="text-6xl font-extrabold tracking-tight">{price}</span>
          <span className="ml-2 text-2xl font-semibold">/month</span>
        </p>
        <p className="mt-8 text-lg text-silver-500">{description}</p>
        <ul role="list" className="mt-8 space-y-6">
          {features.map((feature, index) => (
            <li key={index} className="flex items-center">
              ...
              <span className="ml-4 text-lg text-silver-500">{feature}</span>
            </li>
          ))}
        </ul>
      </div>
